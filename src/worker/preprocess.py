...
def regex_clean_ruidos(text):
    ...
    Ex: ',ola' - '...'  - 'email@alguem'
    """
    return " ".join([t for t in text.strip().split() if re.match(r"[^\W\d]*$", t)])
...
def regex_clean(text):
    ...
    """
    return regex_clean_ruidos(" ".join(RegexpTokenizer(r"\w+").tokenize(text)).lower())
...
def make_tokens(text, lang):
    ...
    word_tokens = word_tokenize(
        regex_clean(
            text
        )  # Vai tirar algumas impurezas do texto, como números e alguns simbolos
    )
